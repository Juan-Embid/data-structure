...
// --------------------------------------------------------------
void resolvererror(vector<int> sv, int num, int mazo) { // O(n) * O(log n) donde n es el tamaño del vector.
    SetTree<int> aux;
    SetTree<int> aux2;

    for (int i = 0; i < num; i++) {
        for (int j = i; j < mazo; j += num) {
            if(!aux.contains(sv[j]))
                aux.insert(sv[j]);
            else { aux.erase(sv[j]); }
        }
        cout << "J" << i + 1 << ": ";
