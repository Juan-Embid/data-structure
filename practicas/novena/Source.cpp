...
/*
  ...
  Componente 2: Juan Embid S�nchez
*/
//@ </answer>
...
/*
 * Clase que implementa diccionarios mediante �rboles binarios de b�squeda
 */
...
int recursive(MapTree<string, int>& r, BinTree<string> const& a) { // O(n)
    ...
    if (!isOperator(a.root())) {
        if (isvariable(a.root())) {
            return r.at(a.root());
        }
        return stoi(a.root());
    }
    left = recursive(r, a.left());
    right = recursive(r, a.right());

    if (a.root() == suma)
        return left + right;
    else if (a.root() == resta)
        return left - right;
